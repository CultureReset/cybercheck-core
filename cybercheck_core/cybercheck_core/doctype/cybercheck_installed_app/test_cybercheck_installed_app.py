...
from cybercheck_core.cybercheck_core.doctype.cybercheck_installed_app.cybercheck_installed_app import (
	CyberCheckInstalledApp,
)


def make_business(name: str = "Test Marina") -> str:
	business = frappe.get_doc(
		{"doctype": "Business", "business_name": name, "slug": name.lower().replace(" ", "-")}
	)
	business.insert(ignore_permissions=True)
	return business.name


def make_install(business: str, app_id: str, **kw) -> CyberCheckInstalledApp:
	values = {
		"doctype": "CyberCheck Installed App",
		"business": business,
		"app_id": app_id,
		"runtime": "frappe",
	}
...
class TestCyberCheckInstalledApp(IntegrationTestCase):
	def setUp(self):
